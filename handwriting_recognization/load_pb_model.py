...
def load_model():
    with tf.gfile.GFile(os.path.join('model', 'gesture.pb'), 'rb') as f:  # 读取模型数据
...
def predict(graph):
    im = Image.open('data/origin/1.jpg')
    mat = np.asarray(im.convert('RGB'))  # 原始图片
    mat = mat.reshape(1, global_vals.resized_image_size, global_vals.resized_image_size, 3)
    mat = mat / 255.
